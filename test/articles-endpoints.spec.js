...
describe("Articles Endpoints", function () {
	...
	before("clean the table", () =>
		db.raw("TRUNCATE blogful_articles, blogful_users, blogful_comments RESTART IDENTITY CASCADE")
	);
	...
	describe(`GET /api/articles/:article_id`, () => {
		...
		context("Given there are articles in the database", () => {
			...
			it("responds with 200 and the specified article", () => {
				...
				return supertest(app).get(`/api/articles/${articleId}`).expect(200, expectedArticle);
			});
		});
		...
		context("Given there are articles in the database", () => {
			...
			it("responds with 204 and updates the article", () => {
				...
				return supertest(app)
					...
					.send(updateArticle)
					.expect(204)
					.then((res) => supertest(app).get(`/api/articles/${idToUpdate}`).expect(expectedArticle));
			});
